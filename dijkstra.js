...
// Nodes with subnodes
const defaultSubNodes = [ 'D', 'G', 'K', 'O', 'P', 'Q', 'S', 'T', 'U']


function drawAristas(...ids) {
    ids.forEach(idBase => {
        ['_1', '_2'].forEach(sufijo => {
            const arista = document.getElementById(idBase + sufijo);
            if (arista) arista.style.strokeOpacity = "0.8";
        });

        // Without suffix
        const aristaSimple = document.getElementById(idBase);
        if (aristaSimple) aristaSimple.style.strokeOpacity = "0.8";
    });
}
...
function drawGraph(path) {
    ...
    for (let index = 0; index < path?.length; index++) {
        ...
        drawAristas(`${idNode}_${nextIdNode}`, `${idNode}${nextIdNode}`);

        // Draw the firt subnode
        drawAristas(`${idNode}_${idNode}1`);

        // Draw the other subnodes
        // The number 6 is because it is the maximum amount of subnodes
        for (let i = 1; i <= 6; i++) {
            if (defaultSubNodes.includes(idNode)){
                drawAristas(`${idNode}${i}_${idNode}${i + 1}`);
                drawAristas(`${idNode}${i}_${nextIdNode}`);
            }
        }
    }
}
